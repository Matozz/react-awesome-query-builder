...
const initialSkin = window._initialSkin || "semi";
const emptyInitValue: JsonTree = { id: uuid(), type: "group" };
const loadedConfig = loadConfig(initialSkin);
let initValue: JsonTree =
  loadedInitValue && Object.keys(loadedInitValue).length > 0 ? (loadedInitValue as JsonTree) : emptyInitValue;
...
const DemoQueryBuilder: React.FC = () => {
  ...
  const runActions = () => {
    ...
    const firstPath = [state.tree.get("id"), ((state.tree.get("children1") as ImmOMap)?.first() as ImmOMap)?.get("id")];
    const lastPath = [state.tree.get("id"), ((state.tree.get("children1") as ImmOMap)?.last() as ImmOMap)?.get("id")];
  ...
  const renderResult = ({ tree: immutableTree, config }: { tree: ImmutableTree; config: Config }) => {
    ...
    return (
      <div>
        ...
        <div>
          spelFormat:
          {spelErrors.length > 0 && <pre style={preErrorStyle}>{stringify(spelErrors, undefined, 2)}</pre>}
        ...
        <div>
          sqlFormat:
          {sqlErrors.length > 0 && <pre style={preErrorStyle}>{stringify(sqlErrors, undefined, 2)}</pre>}
          <pre style={preStyle}>{stringify(sql, undefined, 2)}</pre>
        </div>
        <hr />
        <div>
          <a href="http://jsonlogic.com/play.html" target="_blank" rel="noopener noreferrer">
            jsonLogicFormat
          </a>
          :{logicErrors.length > 0 && <pre style={preErrorStyle}>{stringify(logicErrors, undefined, 2)}</pre>}
          {!!logic && (
            <pre style={preStyle}>
              {"// Rule"}:<br />
        ...
        <div>
          mongodbFormat:
          {mongoErrors.length > 0 && <pre style={preErrorStyle}>{stringify(mongoErrors, undefined, 2)}</pre>}
  ...
  return (
    <div>
      ...
      <div className="query-import-spel">
        ...
        <br />
        {state.spelErrors.length > 0 && <pre style={preErrorStyle}>{stringify(state.spelErrors, undefined, 2)}</pre>}
      </div>
